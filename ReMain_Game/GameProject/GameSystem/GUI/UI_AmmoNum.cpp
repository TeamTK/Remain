...
void UI_AmmoNum::Update()
{
	...
	{
		m_AmmoNum.NumDraw(Vector2D(110, 60), g_pShotgun->GetAmmo(), true);
		m_LoadedAmmoNum.NumDraw(Vector2D(110, 19), g_pShotgun->GetLoadedAmmo(), true);
	}
	else if (g_pHandgun->GetSelect() == EWeapons::eHandgun)
	{
		m_AmmoNum.NumDraw(Vector2D(110, 60), g_pHandgun->GetAmmo(), true);
		m_LoadedAmmoNum.NumDraw(Vector2D(110, 19), g_pHandgun->GetLoadedAmmo(), true);
	}
}